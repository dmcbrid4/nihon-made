...
generated by `scripts/generate-tae-kim-furigana.py`: it tokenizes each
sentence with `fugashi`/UniDic, applies two known casual-register overrides
(何 → なに not なん, 私 → わたし not わたくし), and only keeps the result if
the reconstructed reading exactly matches the deck's own stored reading.
Sentences that don't clear that bar are left with no furigana at all (never
a guessed one) -- measured at 1,987 of 2,076 sentences (95.7%) passing.
Output lives in `src/lib/study/data/tae-kim-furigana.json`, keyed by phrase
id; rerun the script whenever `tae-kim-deck.json` changes (its printed
match-rate is worth checking after a rerun).
