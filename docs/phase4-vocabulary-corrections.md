...
Status: audited-correction implementation complete on `dev` across focused commits;
no automatic merge, push, seed, or production change.
Phase 3's fixed 100-record sample is complete (50 N5, 50 N4). The next required
step is an **Astra, high** independent recheck and fresh holdout. Consult the
...
is not a current release count. Its required corrections are now implemented;
records outside that sample still need independent review. User progress and the
production database were not touched.

## Prioritized implementation checklist

- [x] **Contain known active defects.** Repaired all 33 required decisions from
  the fixed sample (or retained an existing Phase 2 repair), with reproducible
  overrides and target-span checks. This containment does not certify records
  outside the sample.
...
- [x] **Choose among dictionary candidates and intended senses.** Replaced the
...
- [x] **Correct contextual sentence readings.** Extended the existing structured
  ...
  and 私/わたくし in later full-corpus work.
- [x] **Validate target lexeme/sense, not just characters or token boundaries.**
...
- [x] **Apply semantic/pedagogical corrections across affected groups.** Applied
...
- [x] Run focused tests on real failure fixtures and changed behavior; check
