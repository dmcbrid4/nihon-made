...
test("selecting Kana mode from the study-mode picker swaps Today for the Kana home", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "Kana", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Hiragana", exact: true }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", { name: "Katakana", exact: true }),
  ).toBeVisible();
...
test("hiragana Quiz: select one character, answer correctly 5 times, it masters", async ({
  ...
  await page
    .getByRole("button", { name: "Recognition only", exact: true })
    .click();
...
test("marking a character known reflects immediately in progress, and Kana shows on the Progress page", async ({
  page,
}, testInfo) => {
  await page.goto("/kana/hiragana?tab=chart");
  await page.locator(".kana-cell", { hasText: "あ" }).first().click();
  await page.getByRole("button", { name: "Mark known" }).click();
  await expect(page.getByText("あ marked as known.")).toBeVisible();
  await expect(page.getByText("Status: Mastered")).toBeVisible();
  await page.getByRole("link", { name: "Progress", exact: true }).click();
  await page.waitForURL("/progress");
  await expect(
    page.getByRole("heading", { name: "Kana foundations", level: 2 }),
  ).toBeVisible();
  await page.screenshot({
  ...
  expect(state.progress.length).toBe(2); // one character, both directions
  expect(
    state.progress.every(
      (item: { status: string }) => item.status === "mastered",
    ),
  ).toBe(true);
});
