...
- [x] **Vocabulary quality Phase 4: audited-correction implementation** — Terra, high reasoning. The 33 required findings from the fixed sample have reproducible repairs: dictionary disambiguation, 30 canonical noun-plus-する headwords, duplicate retirement, reviewed suffix matching, editorial examples, and contextual ruby overrides. Keep work on `dev`; no production rollout.
- [ ] **Vocabulary quality Phase 4: independent holdout and UI verification** — Astra, high reasoning. Inspect a new reproducible 20-record holdout, review rendered ruby across each vocabulary surface, rerun release checks, and report remaining full-corpus uncertainty before any production rollout.
