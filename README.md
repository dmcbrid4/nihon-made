...
- [x] **Curriculum infrastructure and N5/N4 modes** — GPT-5.6 Terra, high reasoning. The candidate catalog currently contains 730 N5 and 679 N4-only vocabulary records (1,152 of them approved and active), alongside other study content. Modes keep queues, sessions, collections, and progress distinct. The content quality work below remains incomplete.
...
- [x] **Vocabulary quality Phase 2: finish candidate implementation** — mechanically complete, not linguistically complete. Canonical word-reading validation, token/lemma target matching (replacing substring search), real dictionary/attribution provenance, and an explicit approval gate are implemented and regression-tested; see [the plan's Phase 2 entry](docs/vocabulary-quality-plan.md#implementation-and-review-gates) for what this does and does not establish. 1,152 of 1,409 catalog records are approved and active; the rest are quarantined with itemized reasons in [the data-quality report](docs/data-quality-report.md).
...
- [ ] **Vocabulary quality Phase 4: recheck corrections and resolve release blockers** — Terra, high reasoning. The seven fresh-holdout corrections and six missed prose findings are now implemented with regressions. Recheck the repaired data, resolve example attribution or keep affected content quarantined, and reconcile the remaining original audit decisions. Keep work on `dev`; no production rollout.
