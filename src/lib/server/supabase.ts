...
import { hasFreshReauthentication, REAUTH_COOKIE } from "./reauth";
    ...
    cookies: {
      ...
      setAll: (values) => {
        try {
          ...
          /* Proxy refreshes cookies where server components cannot write them. */
        }
      },
    },
  });
...
export async function authenticate(): Promise<
  { userId: string; email: string; status: 200 } | { status: 401 | 403 | 503 }
> {
  const config = getAppConfig();
  if (config.mode !== "database") return { status: 503 };
  try {
    const store = await cookies();
    if (!hasFreshReauthentication(store.get(REAUTH_COOKIE)?.value))
      return { status: 401 };
